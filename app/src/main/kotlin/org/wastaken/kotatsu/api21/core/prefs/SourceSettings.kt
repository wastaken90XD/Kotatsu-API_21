...
import org.wastaken.kotatsu.api21.core.util.ext.sanitizeHeaderValue
...
class SourceSettings(context: Context, source: MangaSource) : MangaSourceConfig {
	...
	override fun <T> get(key: ConfigKey<T>): T {
		return when (key) {
			...
			is ConfigKey.StringConfig -> prefs.getString(key.key, key.defaultValue) ?: key.defaultValue
		} as T
	}
	...
	operator fun <T> set(key: ConfigKey<T>, value: T) = prefs.edit {
		when (key) {
			...
			is ConfigKey.StringConfig -> putString(key.key, value as String)
		}
	}
